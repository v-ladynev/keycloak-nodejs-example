...
class KeyCloakService {
    ...
    getAllPermissions(request) {
        return this.getAccessToken(request)
            ...
            .then(result => KeyCloakService.decodeRptToken(result.body));
    }
    ...
    checkPermission(request, resource, scope) {
        ...
        return this.getAccessToken(request)
            ...
            .then(KeyCloakService.checkEntitlementResponse);
    }
    ...
    createSecurityMiddleware() {
        return (req, res, next) => {
            ...
            if (!permission) {
                console.log("Can't find permission for: %s %s", req.method, req.originalUrl);
    ...
    protectAndCheckPermission(request, response, next, resource, scope) {
        this.keyCloakProtect(request, response, () => this.checkPermission(request, resource, scope)
            .then(next).catch(() => this.keyCloak.accessDenied(request, response)));
    }

    static checkEntitlementResponse(result) {
    ...
    getEntitlementsRequest(accessToken) {
        ...
        return KeyCloakService.requestPromise(options);
    }
